...
def etl_data():
    ...
    start_date = None
    end_date =  None
    df = etl.read(start_date, end_date)
...
def show_donations_by_category(donations_by_category, donations_by_category_quantity):
    ...
    donations_by_category_quantity['Date'] = pd.to_datetime(donations_by_category_quantity['Date'])

    col0, col1, col2, col3, col4 = st.columns(5)
    with col2:
            period = st.selectbox(' ', ['3 Months', 'Month', 'Week', 'Day', 'Year'])
    ...
    if period == 'Year':
        ...
        donations_quantity = donations_by_category_quantity[donations_by_category_quantity['Date'] >= pd.Timestamp.now().floor('D') - pd.DateOffset(months=3)]
    elif period == 'Month':
        donations = donations_by_category[donations_by_category['Date'] > pd.Timestamp.now().floor('D') - pd.DateOffset(months=1)]
